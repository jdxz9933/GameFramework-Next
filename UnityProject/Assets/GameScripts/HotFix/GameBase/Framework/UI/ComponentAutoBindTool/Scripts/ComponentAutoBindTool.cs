...
public class ComponentAutoBindTool : MonoBehaviour {
    ...
    [SerializeField] private string m_ComCodePath;

    [SerializeField] private string m_MountCodePath;
    ...
    public string ComCodePath {
        get { return m_ComCodePath; }
    }

    public string MountCodePath {
        get { return m_MountCodePath; }
    }

    public IAutoBindRuleHelper RuleHelper { get; set; }

    [SerializeField] public string generateCodeHelperTypeName;

    public IAutoGenerateCodeHelper GenerateCodeHelper { get; set; }
#endif
    ...
    public T GetBindComponent<T>(int index) where T : Component {
        ...
        if (bindCom == null) {
            Debug.LogError("类型无效 index:" + index.ToString() + "Type:" + typeof(T));
            return null;
        }
