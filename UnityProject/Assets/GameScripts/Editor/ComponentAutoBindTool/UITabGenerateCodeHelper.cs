...
public class UITabGenerateCodeHelper : AutoGenerateCodeHelper {
    public override string DirectoryPath => "Tab";
    ...
    public override string GenAutoBindMountCode(ComponentAutoBindTool autoBindTool, string className,
        string codePath) {
        string btnStart =
        ...
        for (int i = 0; i < autoBindTool.BindDatas.Count; i++) {
            if (autoBindTool.BindDatas[i].BindCom.GetType() == typeof(UIButtonSuper))
                ...
                inputFuncDict[$"m_{autoBindTool.BindDatas[i].Name}"] =
                    $"{autoBindTool.BindDatas[i].Name}EndEditEvent";
        ...
        if (!File.Exists(filePath)) {
            using (StreamWriter sw = new StreamWriter(filePath)) {
                ...
                sw.WriteLine("\tpublic partial class " + className + " : FormTab\n\t{");
